...
lable_file = '../datasets/MNIST/MNIST/train-labels-idx1-ubyte'
# 读取标签数据集
with open(lable_file, 'rb') as lbpath:
    labels_magic, labels_num = struct.unpack('>II', lbpath.read(8))
    labels = np.fromfile(lbpath, dtype=np.uint8)
 
pic_file = '../datasets/MNIST/MNIST/train-images-idx3-ubyte'
# 读取图片数据集
with open(pic_file, 'rb') as imgpath:
